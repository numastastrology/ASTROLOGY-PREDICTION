...
def analyze(birth_details, chart_data, dasa_info=None):
    ...
    points.append(f"<b>Business Foundation:</b> Entrepreneurial path governed by **{h7_sign}** and **{h7_lord}**.")
    points.append(f"<b>Job Foundation:</b> Service path governed by **{h10_sign}** and **{h10_lord}**.")
    ...
    if m_dignity in ['Exalted', 'Normal'] and s_dignity == 'Debilitated':
        points.append("<b>Entrepreneurial Spark:</b> Strong Mercury triggers effective trade and logic, favoring **Business**.")
        base_score += 15
    elif s_dignity in ['Exalted', 'Normal'] and m_dignity == 'Debilitated':
        points.append("<b>Structural Integrity:</b> Saturn's influence provides the discipline needed for long-term **Service (Job)** careers.")
        base_score -= 15
    else:
    ...
    area_map = {
        7: "Partnership/Business Potential",
        10: "Authority/Job Status",
    ...
    for house_num, area in area_map.items():
        ...
        for planet, pos_str in planetary_pos.items():
             ...
             if p_house == house_num:
                 ...
                 outcome = get_house_outcome(house_num, type='pos' if planet in ['Sun', 'Jupiter', 'Mercury', 'Venus'] else 'neg')
    ...
    if kendras:
        points.append(f"<b>Active Influences:</b> **{', '.join(kendras)}** are in key pivot houses, driving your career choice status.")

    # 5. Final Strategic Recommendation
    if base_score > 65:
        recommendation = "A path in **Business or Independent Trade** is highly favorable based on current planetary strengths."
    elif base_score < 55:
        recommendation = "A stable **Job or Service-oriented Career** is favored for long-term growth and security."
    else:
        recommendation = "A **Balanced Profile**; success is possible in both Job and Business depending on the current period."
