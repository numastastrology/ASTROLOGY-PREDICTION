...
def analyze(birth_details, chart_data, dasa_info=None):
    ...
    points.append(f"<b>Foreign Connection Foundation:</b> Your foreign settlement path is influenced by **{h12_sign}** energy, governed by **{h12_lord}**.")
    ...
    points.append(f"<b>Primary Significator:</b> Rahu (planet of foreign lands) is in the {get_ordinal(r_house)} house in **{r_sign}**.")
    points.append(f"<b>Travel Indicator:</b> Rahu triggers **{get_house_outcome(r_house, type='pos' if r_house in [7, 9, 12] else 'neg')}** in your foreign-related endeavors.")
    ...
    for house_num, area in area_map.items():
        ...
        for planet, pos_str in planetary_pos.items():
             ...
             if p_house == house_num:
                 ...
                 outcome = get_house_outcome(house_num, type='pos' if planet in ['Jupiter', 'Venus', 'Rahu'] else 'neg')
    ...
    if challenges:
        points.append(f"<b>Challenges:</b> **{', '.join(challenges)}** show some restrictions for permanent settlement, necessitating clear documentation.")
    if stability:
        points.append(f"<b>Stability:</b> **{', '.join(stability)}** are well-placed, providing a favorable path for international ventures.")
    ...
    if kendras:
        points.append(f"<b>Active Influences:</b> **{', '.join(kendras)}** are in central houses, actively driving your decisions regarding displacement.")
